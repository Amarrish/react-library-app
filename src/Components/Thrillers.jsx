...
import { getAllBooks,deleteBooks } from '../service/aiiAPI';
...
const Thrillers = () => {
  ...
  useEffect(() => {
   ...
   // Filter books with category "Trending"
   const Investigation = allbooks.filter((book) => book.category === "Investigation");
  return (
    <>
      <div className='mt-3 w-auto'>
        <h3  className='p-3 bg-primary text-light'>Investigation Books</h3>
        <div className='d-flex flex-wrap cards'>
          {Investigation.length > 0 ? (
            Investigation.map((book) => (
              <div key={book.id} className='m-2'>
                <Card style={{ width: '250px',height:'450px',alignItems:'center',boxShadow:'5px solid black'}}>
                  ...
                  <Card.Body>
                    ...
                    <div className='text-center'> <Button variant="primary" onClick={() => handleShowbutton(book.price)}>Borrow</Button> 
    ...
    <Button  type="submit">Confirm your Payment</Button>
            ...
            <p style={{fontWeight:'900', fontSize:'20px'}}><img src={oopsimg} alt="no img" style={{width:'100px',height:'100px', borderRadius:'50%'}} /> There is no Investigation books are availabe now!!!...</p>
          )}
        </div>
      </div>
    </>
